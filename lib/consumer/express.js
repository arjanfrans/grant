  ...
  function connect (req, res) {
    ...
    if (provider.oauth == 1) {
      flow.step1(provider, function (err, data) {
        if (err) return res.redirect((provider.path || '') + provider.callback + '?' + err)
        grant.step1 = data
      ...
      if (provider.callback) {
        res.redirect((provider.path || '') + provider.callback + '?' + qs.stringify(err))
      } else {
  ...
  app.get('/connect/:provider/callback', function (req, res) {
    ...
    function callback (response) {
      if (!provider.transport || provider.transport == 'querystring') {
        res.redirect((provider.path || '') + provider.callback + '?' + response)
      }
      else if (provider.transport == 'session') {
        req.session.grant.response = qs.parse(response)
        res.redirect((provider.path || '') + provider.callback)
      }
    }
